...
def extract_landmarks(video_path: str, progress_bar=None) -> list:
    ...
    with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
        while cap.isOpened():
            ...
            fd = {"frame_number": frame_count, "pose":[], "face":[], "left_hand":[], "right_hand":[]}
            if res.pose_landmarks:
                fd["pose"] = [{"x":lm.x,"y":lm.y,"z":lm.z,"v":lm.visibility} for lm in res.pose_landmarks.landmark]
            if res.face_landmarks:
                fd["face"] = [{"x":lm.x,"y":lm.y,"z":lm.z} for lm in res.face_landmarks.landmark]
            if res.left_hand_landmarks:
                fd["left_hand"] = [{"x":lm.x,"y":lm.y,"z":lm.z} for lm in res.left_hand_landmarks.landmark]
            if res.right_hand_landmarks:
                fd["right_hand"] = [{"x":lm.x,"y":lm.y,"z":lm.z} for lm in res.right_hand_landmarks.landmark]
