...
with tab2:
    ...
    if "Google Drive" in dataset_source:
        ...
        if gdrive_url and st.button("⬇️ Download & extract dataset", key="dl_dataset"):
            ...
            bar       = st.progress(0, text="Downloading dataset from Google Drive…")

            def _reporthook(block, block_size, total):
                if total > 0:
                    bar.progress(min(block * block_size / total, 1.0),
                                 text=f"Downloading… {min(block*block_size, total)/1e6:.0f} / {total/1e6:.0f} MB")
